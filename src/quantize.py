...
import joblib
import os
from utils import compress_to_uint8, decompress_from_uint8, retrieve_model, fetch_data_split, compute_scores
...
def quantize_main():
    ...
    # Quantize coefficients and bias
    ...
    'coef8_min': min_c,
    'coef8_max': max_c,
    'quant_intercept8': q_bias if np.isscalar(q_bias) else q_bias[0],
    'int8_min': min_b if np.isscalar(min_b) else min_b[0],
    'int8_max': max_b if np.isscalar(max_b) else max_b[0]
     }
    ...
    d_bias = decompress_from_uint8(
        np.array([params_quant['quant_intercept8']]),
        np.array([params_quant['int8_min']]),
        np.array([params_quant['int8_max']])
    )[0]
    ...
    # Prediction errors for quantized model
    ...
    # R2 and MSE for quantized model
